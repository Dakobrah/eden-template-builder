...
/** OOP wrapper for stats calculation, cap enforcement, and utility scoring. */
export class StatsCalculatorService {
  /** Categorize an effect ID into its type (stat, resist, bonus, cap, skill, or other). */
  static getEffectCategory(effectId: string): EffectCategory {
  ...
  /** Calculate aggregated stats, resists, bonuses, skills, and utility for a template. */
  static calculateStats(template: Template | null): CalculatedStats {
    const totals = {
    ...
    if (template?.slots) {
      Object.values(template.slots).forEach(item => {
        ...
        Object.entries(item.effects).forEach(([effectId, value]) => {
          const cat = StatsCalculatorService.getEffectCategory(effectId);
          switch (cat) {
  ...
  /** Calculate utility score for a single item. */
  static calculateItemUtility(item: { effects: Record<string, number> }): number {
    let utility = 0;
    Object.entries(item.effects).forEach(([effectId, value]) => {
      const category = StatsCalculatorService.getEffectCategory(effectId);
      switch (category) {
  ...
  /** Generate a human-readable text report for a template. */
  static generateTemplateReport(template: Template, calculated: CalculatedStats): string {
    const lines: string[] = [];
...
// Backward-compatible function exports (existing tests and imports continue to work)
export const getEffectCategory = StatsCalculatorService.getEffectCategory;
export const calculateStats = StatsCalculatorService.calculateStats;
export const calculateItemUtility = StatsCalculatorService.calculateItemUtility;
export const generateTemplateReport = StatsCalculatorService.generateTemplateReport;
